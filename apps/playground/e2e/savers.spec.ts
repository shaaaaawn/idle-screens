...
const ALL_IDS = [
  'black-hole', 'tide', 'toasters', 'dvd', 'warp', 'fish', 'rainstorm', 'hard-rain',
 ...
 * host, survive resize + setPaused, and dispose without errors. The passthrough
 * savers (black-hole, tide, spotlight) may mutate the live page's victims while
 * running, and must restore them on dispose; every non-passthrough saver must
 * touch nothing outside its host.
 */
test('all 28 savers honor the full SaverInstance lifecycle', async ({ page }) => {
