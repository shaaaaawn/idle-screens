...
For shawn-site (a personal deploy), **in-memory or SQLite** is fine.
State is tiny (~1 KB of JSON) and recovery from server restart is
graceful (tabs reconnect and get current state or defaults).
...
1. **Extend `__idleScreens`** with `applyTrack`, `setParam`,
   `getParamSpace`, `getParams`, `setSeed` (core change, no MCP yet)
2. **MCP server** with lifecycle + query tools, in-memory state,
   WebSocket bridge (topology A — local dev first)
3. **Bridge script** for the browser side
4. **Steering tools** once the hook supports `setParam`/`applyTrack`
5. **Topology B upgrade** — SSE, backing store, multi-tab support
   (only if deploying beyond local dev)
