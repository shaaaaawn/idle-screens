...
> (concrete Cloudflare DO implementation spec for topology B),
> [cold-agent-authoring-prompt.md](./cold-agent-authoring-prompt.md)
> (gating experiment #2 prompt template),
...
>
> **Update (July 2026, session 3):** Cloudflare Durable Objects spec written for
> topology B. Key decisions: server code lives in a **separate repo**
> (`idle-server`) — different runtime/deploy/types. This repo owns the
> shared protocol (`@idle-screens/channels-protocol`) and browser bridge
> (`@idle-screens/bridge`). The cold-agent authoring prompt is saved as a template
> for gating experiment #2.
...
### M3 — MCP + shared protocol
- **`@idle-screens/channels-protocol`** (this repo) — shared wire types
  (`ChannelMessage`, `ChannelSnapshot`) used by both the bridge and the server.
  Zero deps. Published to npm first — everything else imports it.
- **`@idle-screens/bridge`** (this repo) — browser WebSocket client (~60 lines)
  that speaks the channel protocol and translates to `__idleScreens` calls.
  Server-agnostic: works with Cloudflare DO, a Node WS server, anything.
- **Local MCP server** (topology A, stdio) — tools: `createSaver(spec)`,
  `setSaver`, `setParam`, `applyTrack`, `getState`, `listSavers`. Resources:
  `screen://state` (live), `screen://schema` (the self-describing authoring
  contract from F1). Can live in this repo as a dev tool.
- Demo: Claude Code connected to the playground — "make it feel like rain is
  coming" → screen changes.

### M4 — The deployed loop
- **Server repo** (`idle-server`, separate repo) — Cloudflare Worker +
  Durable Object + MCP Streamable HTTP endpoint. Full spec in
  [cloudflare-durable-objects-spec.md](./cloudflare-durable-objects-spec.md).
  Imports `@idle-screens/channels-protocol` from npm. Does not depend on core
  — it never renders pixels.
- shawn-site ships `@idle-screens/bridge`, connecting to the deployed Worker.
