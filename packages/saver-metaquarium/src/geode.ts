...
export function buildGeode(spec: GeodeSpec, rng: CrystalRng): GeodeParts {
  ...
  // A stepped disc of planks — round because the throat is.
  const R = FACADE_R, rows = 9;
  const planks = ['#8a5a3c', '#7a4e35', '#96653f'];
  ...
  // Round door, a touch proud of the wall, with its knob.
  const dr = 0.21, dy = -0.26;
  for (let j = 0; j < 5; j += 1) {
    const y = -dr + (j + 0.5) * (2 * dr / 5);
    const w = Math.sqrt(Math.max(0.01, dr * dr - y * y)) * 2;
    vox(voxels, -0.05, dy + y, zf + 0.05, w, 2 * dr / 5 + 0.004, 0.05, j % 2 ? '#4a2f2a' : '#573830');
  }
  vox(glow, 0.08, dy, zf + 0.09, 0.045, 0.045, 0.04, '#ffd76a', false);
  // The lit window, with a cross of muntins so it reads as glass, not a hole.
  const wx = spec.habit === 'tower' ? 0 : 0.26, wy = spec.habit === 'tower' ? 0.24 : 0.14;
  vox(glow, wx, wy, zf + 0.045, 0.17, 0.17, 0.03, WARM, false);
  vox(voxels, wx, wy, zf + 0.065, 0.17, 0.025, 0.02, '#4a2f2a');
  vox(voxels, wx, wy, zf + 0.065, 0.025, 0.17, 0.02, '#4a2f2a');
  if (spec.habit === 'hall') vox(glow, -0.32, 0.14, zf + 0.045, 0.14, 0.14, 0.03, WARM, false);
  // Lamp on a bracket beside the door.
  vox(voxels, -0.3, -0.02, zf + 0.12, 0.03, 0.03, 0.2, '#3a2a28');
  vox(glow, -0.3, -0.08, zf + 0.22, 0.07, 0.09, 0.07, '#ffe2a3', false);
  // Steps from the sill down to the floor, out through the break.
  for (let j = 0; j < 3; j += 1) {
    vox(voxels, -0.05, -0.5 - j * 0.045, zf + 0.12 + j * 0.13, 0.36 + j * 0.08, 0.05, 0.14, j % 2 ? '#59647a' : '#4a566b');
  }
